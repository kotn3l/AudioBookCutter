...
namespace AudioBookCutter
{
    class Command
    {
        ...
        private void cutByTimeSpansIn(List<TimeSpan> times, TimeSpan length, string path, string save)
        {
            ...
            string temp;
            temp = "-ss 00:00:00.0 -to " + ordered[0] + argument + end + 1.ToString().PadLeft(3, '0') + fileFormat + "\"";
            Execute(temp);
            ...
            {
                temp = "-ss " + ordered[i - 1] + " -to " + ordered[i] + argument + end + (i+1).ToString().PadLeft(3, '0') + fileFormat + "\"";
                Execute(temp);
                Log.Information(command + "Cut from {0} to {1} done", ordered[i - 1], ordered[i]);
                temp = "";
                i++;
            }
            temp = "-ss " + ordered[i-1] + " -to "+ length + argument + end + (i+1).ToString().PadLeft(3, '0') + fileFormat + "\"";
            Execute(temp);
